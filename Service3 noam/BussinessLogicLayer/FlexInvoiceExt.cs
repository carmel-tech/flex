...
namespace BussinessLogicLayer
{
    public class FlexInvoiceExt
    {
        ...
        private void openConnection(OdbcConnection DbConnection)
        {
            ...
            {
                throw (new Exception("Could not connect to DB with \n" + ex.Message));
            }
        }
//        public bool FetchFromDB(string dsn = "DSN=BAAN", bool debug = false)
        public bool FetchFromDB(OdbcConnection DbConnection, bool debug ,EventLog logger)
        {
  ...
  //          using (OdbcConnection DbConnection = new OdbcConnection(dsn))
            {
                ...
                //Query #1
                logger.WriteEntry("invoice number - " + InvoiceNumber);
                ...
                using (OdbcCommand DbCommand = DbConnection.CreateCommand())
                {
                    ...
                    try
                    {
                        ...
                        logger.WriteEntry("success 1- " + DbCommand.CommandText);
                    }
                    catch (Exception ex)
                    {
                        ...
                        logger.WriteEntry("fail 1- " + DbCommand.CommandText);
                        return false;
                    }
                    if (debug) Console.WriteLine("4");
                    if (reader.Read())
                    {
                        logger.WriteEntry("success 2- " + DbCommand.CommandText);
                    ...
                    try
                    {
                        logger.WriteEntry("success 3- " + cmd);
                        reader = DbCommand.ExecuteReader();
                    }
                    catch (Exception ex)
                    {
                    ...
                    if ( reader.Read() )
                    {
                        logger.WriteEntry("success 4- " + cmd);
                    ...
                    {
                        reader = DbCommand.ExecuteReader();
                        logger.WriteEntry("success 5- " + cmd);
                    }
                    catch (Exception ex)
                    {
                    ...
                    if ( reader.Read() )
                    {
                        logger.WriteEntry("success 6- " + cmd);
                    ...
                    {
                        reader = DbCommand.ExecuteReader();
                        logger.WriteEntry("success 7- " + cmd);
                    }
                    ...
                    if (reader.Read())
                    {
                        logger.WriteEntry("success 8- " + cmd);                   
                ...
                //Query #4
                    ...
                    {
                        reader = DbCommand.ExecuteReader();
                        logger.WriteEntry("success 9- " + cmd);
                    }
                    catch (Exception ex)
                    {
                    ...
                    if ( reader.Read() )
                    {
                        logger.WriteEntry("success 10- " + cmd);
                    ...
                    cmd = "select DSLS041.t_epos, DSLS045.t_dqua, DSLS045.t_pric, DSLS045.t_amnt, DSLS045.t_item, IITM001.t_dsca, TTCMCS041.t_dsca,"+
                        " DSLS040.t_cotp, DSLS040.t_refa, DSLS040.t_odat, TCMCS013.t_dsca, DSLS040.t_orno, DSLS045.t_damt_1 ,DSLS040.t_refb  " +
                        " from baandb.ttdsls045{0} as DSLS045, baandb.ttiitm001{0} as IITM001, baandb.ttdsls040{0} as DSLS040, baandb.ttdsls041{0} as DSLS041, baandb.ttcmcs041{0} as TTCMCS041, baandb.ttcmcs013{0} as TCMCS013 where  DSLS045.t_invn={1} and IITM001.t_item=DSLS045.t_item and DSLS041.t_item=DSLS045.t_item and DSLS040.t_orno=DSLS045.t_orno and (DSLS041.t_pono=DSLS045.t_pono and DSLS041.t_orno=DSLS045.t_orno) and TTCMCS041.t_cdec=DSLS040.t_cdec and TCMCS013.t_cpay=DSLS040.t_cpay";
                    DbCommand.CommandText = String.Format(cmd, CompanyCode, InvoiceNumber);
                    ...
                    {
                        reader = DbCommand.ExecuteReader();
                        logger.WriteEntry("success 11- " + cmd);
                    }
                    catch (Exception ex)
                    {
                    ...
                    while (reader.Read())
                    {
                        ...
                        logger.WriteEntry(reader.GetDecimal(1).ToString());
                        //try
                        //{
                            decimal qtyD = reader.GetDecimal(1);
                        //    string qty = reader.GetDecimal(1).ToString();
                        //    if (qty.IndexOf('.') != -1 && (qty.IndexOf('.') + 3) <= qty.Length)
                        //        //     line.UnitQuantity = Math.Round(qtyD, 2).ToString();
                        //        //     logger.WriteEntry("  line.UnitQuantity  - " + line.UnitQuantity);
                        ...
                        //}
                        //catch { }
                            line.UnitQuantity = Math.Round(qtyD, 2, MidpointRounding.AwayFromZero).ToString();
                            line.ItemPriceBruto = Math.Round(reader.GetDecimal(2), 4, MidpointRounding.AwayFromZero);
                            line.LineSum = Math.Round(reader.GetDecimal(3), 2, MidpointRounding.AwayFromZero);
                        ...
                        line.PartNumber = "";
                        line.RefZicui = reader.GetString(13) == null || reader.GetString(13) == "" ? "" : reader.GetString(13);
        ...
        public bool SerializeToXML(string path,string packingSlip, bool debug,EventLog logger)
        {
...
//            inv_xml.Envelope.Header.PaymentTerms = "";
            ...
            inv_xml.Envelope.Header.Reference = new InvoiceXMLEnvelopeHeaderReference[3/*TotalLines*/];
            ...
            foreach (InvoiceLine line in Lines)
            {
                ...
                if (counter == 0)
                {
                    //Ayala -Add new Refernce for Zicui 26/03/2015
                    InvoiceXMLEnvelopeHeaderReference Reference3 = new InvoiceXMLEnvelopeHeaderReference();
                    Reference3.RefNo = line.ReferenceNumber;
                    Reference3.RefDate = System.String.Format("{0:yyyy-MM-dd}", line.ReferenceDate);
                    Reference3.RefType = InvoiceXMLEnvelopeHeaderReferenceRefType.purchaseOrder;
                    inv_xml.Envelope.Header.Reference[counter] = Reference3;
                    //end change  Ayala -Add new Refernce for Zicui 26/03/2015 
                    InvoiceXMLEnvelopeHeaderReference Reference = new InvoiceXMLEnvelopeHeaderReference();
                    Reference.RefNo = line.RefZicui;
                    Reference.RefType = InvoiceXMLEnvelopeHeaderReferenceRefType.purchaseOrderAck;
                    inv_xml.Envelope.Header.Reference[counter+1] = Reference;
                  
                  //  if (inv_xml.Envelope.Header.InvoiceType.ToLower().Equals("SCN".ToLower()))
                 //   {
                        ...
                        inv_xml.Envelope.Header.Reference[counter + 2] = Reference2;
               //     }
                    ...
                    ////2 changes in Reference2 and up from 2->3 
                    //////Ayala -Add new Refernce for Zicui 26/03/2015
                    ////InvoiceXMLEnvelopeHeaderReference Reference3 = new InvoiceXMLEnvelopeHeaderReference();
                    ////Reference3.RefNo = line.RefZicui;
                    ////Reference3.RefType = InvoiceXMLEnvelopeHeaderReferenceRefType.invoice;
                    ////inv_xml.Envelope.Header.Reference[counter + 1] = Reference3;
                    //////end change  Ayala -Add new Refernce for Zicui 26/03/2015
                ...
                //        LineSumStr = Math.Round(decimal.Parse(LineSumStr), 2).ToString();
                //        logger.WriteEntry("LineSumStr - " + LineSumStr);

                //    }
                //}
                logger.WriteEntry("level 5", System.Diagnostics.EventLogEntryType.Information);
                    ////////////////////////////
                ...
                Detailes.UnitsQty = line.UnitQuantity;
            ...
            {
                logger.WriteEntry("Exception in serialization method: " + e.ToString(), System.Diagnostics.EventLogEntryType.Error);
                if (debug)
                {
